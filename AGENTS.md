...
### 2. 크론은 환경변수가 없으면 조용히 실패한다

`/api/cron/*`는 `Authorization: Bearer $CRON_SECRET`을 검증한다. `CRON_SECRET` 또는 `OPENAI_API_KEY`가 없으면 200 OK를 반환하면서 아무것도 저장하지 않는다. **로컬뿐 아니라 Vercel 프로젝트 환경변수에도 등록해야 한다.**

크론 응답의 `failed`/`feedsFailed`/`errors`를 반드시 확인하라. 실패를 조용히 넘기지 않는 것이 이 라우트의 설계 의도다.

### 3. 이미지는 2단계 정리 파이프라인을 탄다
...
`dark:` 클래스를 쓰지 않는다. 라이트 모드 전용이다. (도입 검토는 `PLAN.md` T-405)
...
- 본문 `gray-900`, 부가 `gray-500`, 비활성 `gray-400`, 기본 보더 `gray-200`
